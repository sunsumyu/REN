...
* **输出提纯重构后的完美思维链**：
\"\"\"
探究白头翁的临床应用，需首先锁定其在中药性味归经中的核心定位：苦寒，入大肠经。苦寒之性决定了其具有强效的清热解毒、凉血之功。既然直达大肠血分，那么其最核心的对症病理必然是肠道湿热毒盛、伤及血络所致的疾病。由此推导，临床上以腹痛、里急后重、下痢脓血为特征的“热毒血痢”（如现代医学的细菌性痢疾、阿米巴痢疾），正是白头翁的绝对主治靶点。进一步延伸，既然其擅长清下焦湿热与解毒杀虫，那么当湿热下注侵袭女性生殖系统时，引发的阴痒、带下黄稠等症，亦可利用其清热燥湿杀虫之效进行辨治。因此，其主治方向完全由其“清热解毒、凉血止痢”的核心功效严密推演而来。
\"\"\"
"""
...
3. 🧠 【维度三：逻辑深度与思维熵 (logical_depth_score - 0到100分)】
   ...
   - **🚨 负面惩罚样例与控分机制（严防阿谀奉承与对长文本的虚高评价）**：
     - *类型一（概念名词堆砌而无临床闭环）*：单纯罗列高深医学名词，未紧扣临床矛盾展开因果推导，得分**绝对不能超过 75 分**。
     - *类型二（低级做作的自我纠偏）*：写出无医学逻辑价值的注水废话（如：“等一下，不对，刚才看错了”等生硬否定句），扣除 **20-40 分**。
     - *类型三（🚨静态陈述与缺乏思维摩擦惩罚）*：如果思维链读起来像是一篇四平八稳的“教科书标准答案”或“百度百科”，全篇都是“A是B”、“C的机制是D”的静态陈述句，而**严重缺乏“推断性疑问、假设排查、因果连接词（如：既然、然而、进一步来看、由此推导）”等动态探索轨迹**，此维度得分**必须扣至 70 分以下**！
...
def is_catastrophic_format_collapse(text: str) -> bool:
    ...
    # 🧠 精细化 RAG 工程泄露硬网关（对齐百度/百川的漏检拦截标准）
    # 仅拦截带有工程痕迹的“参考资料”、“原始检索”等流水线污染词，允许“临床文献/临床研究/学术界”等严谨表达
    leakage_patterns = [
