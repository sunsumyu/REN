# -*- coding: utf-8 -*-
"""
专用于存放思维链提纯净化、描述性引导以及裁判质检的超长提示词模板及 Few-shot 示范。
"""
...
def get_purify_system_prompt(planner: str, simplify: bool = False) -> str:
    """动态生成系统提示词，注入特异性视角红线拦截与极简指令支持"""
    extra_redlines = (
        "\n8. 🚨 【严禁学术伪造与高仿真事实幻觉（Anti-Hallucination Redline）】:\n"
        "   - 绝对禁止捏造任何未经临床验证的临床试验名称（例如：“早期人体挑战试验”）。\n"
        "   - 绝对禁止无依据地捏造特定的限制人群或禁忌症分类（例如：无明确说明书依据地泛化或捏造“绝经期前女性禁用”）。\n"
        "   - 绝对禁止篡改官方药品标准号、批准文号或质量准则代号（例如：将真实的‘YBZ’写错或编造为‘Y8Z’）。所有标准文号必须与原始 refs 锚点数据保持绝对字面吻合。\n"
    )
    
    extra_simplify = ""
    if simplify:
        extra_simplify = (
            "\n\n### 💡 【极简推理特别指令 (Simplify Rationale Directive)】:\n"
            "当前问答涉及较简单直接的临床事实/定义（如剂量、用法、药理分类或简单禁忌查询），"
            "因此你必须极度简化思维链，直奔主题。严禁脑补复杂的分子靶点机制或生化建模逻辑，"
            "只对核心问题展开 1-2 步直接的临床逻辑推演，用最精简的几句话讲清医学事实即可。"
        )
...
1. 🟢 【维度一：语义纯净度 (semantic_purity_score - 0到100分)】
   ...
   - **工程泄露零容忍**：如果在文中出现 '根据提供的参考资料'、'检索到'、'如上所述' 等 AI 语言风格，纯净度扣 50 分。
...
2. 🩺 【维度二：医学事实严谨度 (medical_rigor_score - 0到100分)】
   ...
   - **动态实体与数值核验（去个例化与自适应纠偏）**：
     ...
     - **🚨【绝对红线：严打学术伪造与高仿真幻觉】**：裁判大模型必须动用你知识库中最精密的生化与临床药理知识，严密审视提纯后的思维链是否存在“凭空捏造不存在的临床研究或试验（如‘早期人体挑战试验’）、伪造或篡改说明书的官方标准代号（如将‘YBZ’写错或编造为‘Y8Z’）、捏造无依据的限制或慎用患者人群（如无依据编造‘绝经期前女性禁用’）、凭空捏造分子结构骨架（例如将噻唑并吡啶记错为噻吩并吡啶）”等行为。一旦发现模型在“没有确切 refs 证据且缺乏医学公理支撑”的前提下，使用极为自信的伪学术行话进行事实虚构，判定为“高仿真幻觉违规”，此维度（医学事实严谨度）得分一票否决直接扣至 50 分以下并驳回重写！
     - **事实幻觉惩罚**：如果原题明确指定了特定的患者人群（如孕妇、儿童）、特定分期或禁止特定检查（如：禁忌使用X），而提纯后的文本依然在推荐或脑补错误的临床指南与禁忌，立即给出 0 分。
...
- 您必须且只能输出符合以下 JSON Schema 的规范 JSON 串，绝对不要包裹在 markdown ``` 块中，不要有任何额外文字：
{
  ...
  "factual_errors": ["具体错误描述1", "具体错误描述2"], // 必须在此清晰、结构化地列出发现的所有医学事实硬伤、化学术语错词、捏造数据或学术伪造（如：将兰索拉唑的亚磺酰基错写成了硫醚基团）。若完全无错误，输出空列表 []。
  "conflict_detected": bool, // 重点判定：评估净化后的思维链（或最终回答）与原始确证医学文献事实/常识之间是否存在任何实质性冲突、矛盾或篡改（例如原始事实为“小便不利”，思维链或回答写成了“小便自利”；或者原始事实指出某药不应与某药合用，而思维链认为可以合用）。若存在冲突，必须设为 true，否则为 false。
  "conflict_description": "描述冲突的实质内容，如果没有冲突则为空字符串",
  "conflict_details": { // 冲突细节。如果没有冲突，各字段设为 null，或者直接设为 null
    "field_name": "发生冲突的具体字段名或医学术语/药物名称",
    "original_value": "原始文献事实中的真实值/正确表述",
    "corrected_value": "思维链或回答中写错的/篡改的错误值",
    "evidence": "来自原始文献事实的真实依据"
  },
  "reason": "严谨细致的扣分或优胜理由说明（写明具体扣分点，如‘发现字面打印了阶段标题’或‘通篇没有问号，流于平淡的科普文章叙事’）",
  "improvement_suggestions": "针对本次评估发现的缺陷（或可优化空间），给出具体的下一步改进与优化建议（如：如何改善药理逻辑、需补充何种文献依据、如何移除特定残留词汇、或未来模型微调时应当注意的事项）"
}
"""
