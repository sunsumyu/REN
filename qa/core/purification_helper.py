...
**【绝对禁忌红线】**：
绝对禁止在思维链的任何地方提及、暗示或讨论与系统工程实现相关的“信息获取途径”（如内部图谱数据库、RAG检索管线、Refs引用标签、实体库定义、后台数据源）或“数据结构处理”（如子问题拆解、Answer Body、推理链步骤）。任何此类工程概念一旦流出，均判定为严重泄露，该生成将被物理拦截。你应当假定你脑海里拥有最完备的医学专家常识，直接开始纯粹的临床与药理因果演绎。注：你被完全允许且高度鼓励以专业的口吻客观引用科学共识与临床实体来源，如“根据药品说明书”、“临床文献报道”、“诊疗指南推荐”等，这符合严谨的循证医学表述。
...
3. 🚨 【严禁 RAG 边界抱怨与参数知识激活（Anti-RAG Complaining & Parametric Activation）】：
   - **绝对禁止在思考链中写出任何关于后台检索系统或检索文档上下文局限性的工程“抱怨”与“免责”表述！**（如：“由于检索资料有限无法...”、“本条目中没有关于X的数据”）。对于科学证据的正常不确定性客观声明（如“目前尚缺乏针对该配伍的大样本双盲临床对照试验”）是完全允许且被鼓励的，但必须避免暴露检索系统的瓶颈。
...
1. 🟢 【维度一：语义纯净度 (semantic_purity_score - 0到100分)】
   - **核心判定逻辑（元叙述语义自检法）**：
     ...
     - **元叙述的语义判定特征（一旦触碰，此维度得分直接降至 60 分以下并驳回重写）**：
       ...
       2. 文本中是否提到了任何指代系统工程实现的词汇，如“检索图谱”、“实体库”、“外部参考数据”、“数据源”或“refs”等？（注：如果文本中是客观地引用“说明书记载”、“临床文献报道”或“诊疗指南推荐”，则视为完全合规的学术引用，绝对允许且不属于违规泄露）。
   ...
   - **绝对禁止词汇（工程流水线与抱怨，检出一个即扣 20 分）**：
     ...
     - *工程抱怨与无逻辑拒答*：任何体现暴露后台检索管线瓶颈的“抱怨”（包括但不限于 `"由于检索资料有限"`, `"检索条目中未提及"` 等），或在完全可以通过药理/病理生理常识进行合理推演时采取无逻辑的纯粹拒答（如 `"无法回答"`、`"不知道"`）。
   ...
   - **白名单词汇（允许且鼓励的循证/临床逻辑词，禁止扣分）**：
     `"证据"`, `"循证"`, `"排除"`, `"无关"`, `"忽略"`, `"临床指南"`, `"药理关联"`, `"机制"`, `"诊断"`, `"自我修正"`, `"说明书"`, `"文献"`, `"报道"`, `"临床数据"`.
