...
async def send_single_request(client: httpx.AsyncClient, req_id: int, model: str, is_structured: bool = False) -> dict:
    ...
    if is_structured:
        ...
        if is_openai:
            ...
            # 【修复方法】：完全效仿生产环境 api_client 的最佳实践
            # 不发送 response_format 字段，绕过网关乱拼 json_schema 的转译 Bug
            import json
            schema_str = json.dumps(TestPlan.model_json_schema())
            messages.insert(0, {
                "role": "system", 
                "content": f"You must output ONLY a valid JSON object matching this schema:\n{schema_str}"
            })
